  ...
  if(endNodes.length !== 0){
    ...
    if(!endNodeNotEdge){
      ...
      if (flag){
        ...
        return [flag, 'به node پایان حتما باید node های از نوع Announcement متصل باشد.'];
      }
    } else {
      return [false, 'به node پایان حتما باید node های از نوع Announcement متصل باشد.'];
    }
  }
  else {
  ...
  if(forwardsNode.length !== 0){
    ...
    if(!forwardNodeNotEdge){
      ...
      forwardsEdge.forEach(item=>{
        sourceOfForwardsNode.push(nodes.filter(elem=> elem.id === item.source))
      })
      ...
      if(flag){
        ...
        return [flag, 'به node ارجاع حتما باید node های از نوع Announcement متصل باشد.'];
      }
    } else {
      return [false, 'به node ارجاع حتما باید node های از نوع Announcement متصل باشد.'];
    }
  }
  else {
    return [true, null];
